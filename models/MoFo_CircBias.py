...
class MoFo_CircBias_Backbone(nn.Module):
    def __init__(self, dim, cycle, head, lambda_init=0.1, use_causal_mask=False):
        super(MoFo_CircBias_Backbone, self).__init__()
        self.dim = dim
        self.attn = MoFo_CircBias_Attention(dim, cycle, head, lambda_init, use_causal_mask)
        self.ffn = SwiGLU_FFN_CB(dim, dim)

        self.attn_norm = RMSNorm_CB(dim, bias=True)
        self.ffn_norm = RMSNorm_CB(dim, bias=True)
...
class MoFo_CircBias_Attention(nn.Module):
    def __init__(self, dim, cycle=24, head=4, lambda_init=0.1, use_causal_mask=False):
        super(MoFo_CircBias_Attention, self).__init__()
        self.head_num = head
        ...
        self.func_a1 = nn.Parameter(torch.zeros(1, cycle))
        self.func_a2 = nn.Parameter(torch.zeros(cycle, 1))
        self.func_b1 = nn.Parameter(torch.zeros(1, cycle))
        self.func_b2 = nn.Parameter(torch.zeros(cycle, 1))

        distance = torch.abs(torch.arange(cycle).unsqueeze(1) - torch.arange(cycle).unsqueeze(0))
        self.diff = nn.Parameter(
            torch.abs(torch.min(distance % cycle, (-distance) % cycle)).float(),
            requires_grad=False
        )
    ...
    def func(self):
        a = torch.sigmoid(self.func_a1 @ self.func_a2)
        b = torch.sigmoid(self.func_b1 @ self.func_b2) * self.cycle
    ...
    def forward(self, x):
        ...
        periodic_bias = torch.log(self.func())
        attn_scores = attn_scores + periodic_bias.unsqueeze(0).unsqueeze(0)
...
class RMSNorm_CB(nn.Module):
    def __init__(self, d, p=-1., eps=1e-8, bias=False):
        super(RMSNorm_CB, self).__init__()
        self.eps = eps
        self.d = d
        self.p = p
        self.bias = bias
...
class MoFo_CircBias(nn.Module):
    def __init__(self, configs, individual=False):
        super(MoFo_CircBias, self).__init__()
        ...
        self.norm = RevIN_CB(self.channels, eps=1e-5, affine=True)
        ...
        self.MoFo_Backbone = nn.Sequential(*[
            MoFo_CircBias_Backbone(self.dim, self.periodic, self.head, lambda_init, use_causal_mask)
    ...
    def get_circulant_reg_loss(self):
        ...
        for module in self.MoFo_Backbone:
            if isinstance(module, MoFo_CircBias_Backbone):
...
class Linear_CB(nn.Module):
    def __init__(self, seq_len, pred_len, bias=False):
        super(Linear_CB, self).__init__()
...
class RevIN_CB(nn.Module):
    def __init__(self, num_features: int, eps=1e-5, affine=True):
        super(RevIN_CB, self).__init__()
