...
class TransformerAdapterCirculantDP(ModelBase):
    def __init__(self, model_name, model_class, **kwargs):
        super(TransformerAdapterCirculantDP, self).__init__()
        self.config = TSLibConfigDP(**kwargs)
    ...
    def forecast_fit(
        ...
        **kwargs,
        ...
        for epoch in range(config.num_epochs):
            time1 = time.time()
            self.model.train()
            epoch_pbar = tqdm(train_data_loader, desc=f"Epoch {epoch+1}/{config.num_epochs}", leave=False)
            for i, (input, target, input_mark, target_mark) in enumerate(
                ...
                circulant_reg_loss = torch.tensor(0.0, device=device)
                if hasattr(self.model, 'get_circulant_reg_loss'):
                    circulant_reg_loss = self.model.get_circulant_reg_loss()

                loss = loss_main + circulant_reg_loss

                loss2 = torch.mean((torch.fft.rfft(output, dim=1) - torch.fft.rfft(target, dim=1)).abs(), dim=(0, 1))
                loss2_ = loss2.detach()
                loss_freq = torch.sum((loss2 / loss2_) * loss2_.max())

                total_loss = loss + loss_freq

                total_loss.backward()
                optimizer.step()
                epoch_pbar.set_postfix(loss=total_loss.item(), reg=circulant_reg_loss.item())
            time2 = time.time()
            ...
            if train_ratio_in_tv != 1:
                ...
                print(f"Epoch {epoch+1}/{config.num_epochs} | Loss: {total_loss.item():.6f} | Reg: {circulant_reg_loss.item():.6f} | Time: {time2-time1:.2f}s")
...
def generate_model_factory(
    ...
    def model_factory(**kwargs) -> TransformerAdapterCirculantDP:
        return TransformerAdapterCirculantDP(model_name, model_class, **kwargs)
...
def transformer_adapter_circulant_dp(model_info: Type[object]) -> object:
